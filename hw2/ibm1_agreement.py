...
from better_initialization import initialize_prob_by_appearance
...
bitext_rev = [[sentence.strip().split() for sentence in pair] for pair in zip(open(e_data), open(f_data))][:opts.num_sents]
...
def agreement_e_step(bitext, t_ef, t_fe):
    ...
    for (origin, target) in bitext:
        ...
        for t_w in target:
            for o_w in origin:
                agreement_prob = (t_ef[o_w][t_w] * t_fe[t_w][o_w])
...
def run_joint_ibm(bitext, num_iter=5):
    t_ef = initialize_prob_by_appearance(bitext)  # E-to-F
    t_fe = initialize_prob_by_appearance(bitext_rev)  # F-to-Eß
    ...
    for (origin, target) in bitext:
        for i, o_w in enumerate(origin):
            ...
            for j, t_w in enumerate(target):
                combined_prob = (t_ef[o_w][t_w] * t_fe[t_w][o_w])
...
def align_joint_with_threshold(bitext, t_ef, t_fe, threshold):
    for (origin, target) in bitext:
        for i, o_w in enumerate(origin):
            for j, t_w in enumerate(target):
                combined_prob = (t_ef[o_w][t_w] * t_fe[t_w][o_w]) ** 0.5
                if combined_prob > threshold:
                    sys.stdout.write(f"{i}-{j} ")
        sys.stdout.write("\n")


def align_joint_with_two_max(bitext, t_ef, t_fe):

    for (origin, target) in bitext:
        probs = defaultdict(lambda: defaultdict(float))
        for  i, o_w in enumerate(origin):
            for j, t_w in enumerate(target):
                probs[i][j] = (t_ef[o_w][t_w] * t_fe[t_w][o_w]) ** 0.5
        ...
        for j in probs[i].keys():
            ...
            for i in probs.keys():
                if probs[i][j] > max_prob:
                    ...
                    max_index = i
            if max_prob > .5:
                sys.stdout.write(f"{max_index}-{j} ")
...
align_joint_with_threshold(bitext, t_ef, t_fe, .35)
